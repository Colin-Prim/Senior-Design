"""
This module operates in a left-handed coordinate system, with Z pointing up and Y pointing forward.
It defines functions for normalizing vectors, converting between direction cosine matrices (DCM)
and quaternions, and performing quaternion arithmetic operations.
"""
...
def normalize(x):
    ...
    Returns:
        The normalized vector, or a scaled vector if the magnitude is very small.
    """
...
def dcm_from_axis(x_dir, y_dir, z_dir, order):
    """
    Constructs a Direction Cosine Matrix (DCM) from axis vectors following a specified rotation order.

    Args:
        x_dir: Direction vector for the x-axis.
        y_dir: Direction vector for the y-axis.
        z_dir: Direction vector for the z-axis.
        order: String specifying the rotation order ('yzx', 'yxz', etc.).

    Returns:
        A 3x3 matrix representing the DCM.
    """
    assert order in ['yzx', 'yxz', 'xyz', 'xzy', 'zxy', 'zyx']

    # Set up axis dictionary and ordering indices based on rotation order
    ...
    # Normalize primary axis based on order
    axis[order[0]] = normalize(axis[order[0]])
    
    # Define the second axis by taking the cross product of the other two axes
    axis[order[1]] = normalize(np.cross(
        axis[name[(idx1 + 1) % 3]], axis[name[(idx1 + 2) % 3]]
    ))

    # Define the third axis to maintain orthogonality
    axis[order[2]] = normalize(np.cross(
        axis[name[(idx2 + 1) % 3]], axis[name[(idx2 + 2) % 3]]
    ))

    # Form the DCM matrix with x, y, z vectors as rows
...
def dcm2quat(dcm):
    ...
    Args:
        dcm: A 3x3 DCM representing orientation.
    ...
    # Handle different cases based on the trace to avoid division by zero
    if tr > 0:
        ...
        # If trace is non-positive, determine the largest diagonal element and normalize
        d = np.diag(dcm)
