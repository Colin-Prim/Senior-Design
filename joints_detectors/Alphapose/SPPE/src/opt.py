...
# Set up an argument parser to handle different configurations for training
...
parser.add_argument('--trainIters', default=0, type=int,
                    help='Total train iterations')
parser.add_argument('--valIters', default=0, type=int,
                    help='Total valid iterations')
...
parser.add_argument('--baseWidth', default=9, type=int,
                    help='Base width for residual blocks')
parser.add_argument('--cardinality', default=5, type=int,
                    help='Cardinality for residual blocks')
parser.add_argument('--nResidual', default=1, type=int,
                    help='Number of residual modules at each location in PyraNet')
...
parser.add_argument('--port', dest='port', help='Port for distributed training server')

# Parse the arguments
opt = parser.parse_args()

# If continuing from a saved experiment, load the saved options
if opt.Continue:
    # Load previous experiment configuration
    opt = torch.load("../exp/{}/{}/option.pkl".format(opt.dataset, opt.expID))
    opt.Continue = True
    opt.nEpochs = 50  # Adjust epoch count if necessary
    print("--- Continue ---")
