import argparse
import os
import sys
import zipfile
from glob import glob
from shutil import rmtree

import h5py
import numpy as np

# Adding the parent directory to sys.path for imports
sys.path.append('../')

# Import necessary modules from project
from common.h36m_dataset import Human36mDataset
from common.camera import world_to_camera, project_to_2d, image_coordinates
from common.utils import wrap

# Define output filenames and subjects for the Human3.6M dataset
output_filename = 'data_3d_h36m'
output_filename_2d = 'data_2d_h36m_gt'
subjects = ['S1', 'S5', 'S6', 'S7', 'S8', 'S9', 'S11']  # List of Human3.6M subjects

if __name__ == '__main__':
    # Ensure the script is run from the "data" directory
    if os.path.basename(os.getcwd()) != 'data':
        print('This script must be launched from the "data" directory')
        exit(0)

    # Define command-line arguments for dataset sources
    parser = argparse.ArgumentParser(description='Human3.6M dataset downloader/converter')
    parser.add_argument('--from-archive', default='', type=str, metavar='PATH', help='convert preprocessed dataset')
    parser.add_argument('--from-source', default='', type=str, metavar='PATH', help='convert original dataset')
    args = parser.parse_args()

    # Check for mutually exclusive arguments
    if args.from_archive and args.from_source:
        print('Please specify only one argument')
        exit(0)

    # Exit if output dataset already exists
    if os.path.exists(output_filename + '.npz'):
        print('The dataset already exists at', output_filename + '.npz')
        exit(0)

    # Processing the preprocessed dataset from a ZIP archive
    if args.from_archive:
        print('Extracting Human3.6M dataset from', args.from_archive)
        
        # Extract the dataset from the provided ZIP file
        with zipfile.ZipFile(args.from_archive, 'r') as archive:
            archive.extractall()

        print('Converting...')
        output = {}
        
        # Process each subject
        for subject in subjects:
            output[subject] = {}
            file_list = glob('h36m/' + subject + '/MyPoses/3D_positions/*.h5')
            assert len(file_list) == 30, f"Expected 30 files for subject {subject}, got {len(file_list)}"
            
            # Process each action's 3D positions
            for f in file_list:
                action = os.path.splitext(os.path.basename(f))[0]

                # Skip corrupted video
                if subject == 'S11' and action == 'Directions':
                    continue

                with h5py.File(f) as hf:
                    positions = hf['3D_positions'][:].reshape(32, 3, -1).transpose(2, 0, 1)
                    positions /= 1000  # Convert millimeters to meters
                    output[subject][action] = positions.astype('float32')

        # Save the 3D positions to a compressed .npz file
        print('Saving...')
        np.savez_compressed(output_filename, positions_3d=output)

        # Clean up extracted files
        print('Cleaning up...')
        rmtree('h36m')

        print('Done.')

    # Processing from the original Human3.6M dataset source
    elif args.from_source:
        print('Converting original Human3.6M dataset from', args.from_source)
        output = {}

        from scipy.io import loadmat

        for subject in subjects:
            output[subject] = {}
            file_list = glob(args.from_source + '/' + subject + '/MyPoseFeatures/D3_Positions/*.cdf.mat')
            assert len(file_list) == 30, f"Expected 30 files for subject {subject}, got {len(file_list)}"
            
            # Process each file and action in the dataset
            for f in file_list:
                action = os.path.splitext(os.path.splitext(os.path.basename(f))[0])[0]

                # Skip corrupted video
                if subject == 'S11' and action == 'Directions':
                    continue

                # Standardize action naming conventions
                canonical_name = action.replace('TakingPhoto', 'Photo').replace('WalkingDog', 'WalkDog')

                hf = loadmat(f)
                positions = hf['data'][0, 0].reshape(-1, 32, 3)
                positions /= 1000  # Convert millimeters to meters
                output[subject][canonical_name] = positions.astype('float32')

        # Save the 3D positions to a compressed .npz file
        print('Saving...')
        np.savez_compressed(output_filename, positions_3d=output)

        print('Done.')

    else:
        print('Please specify the dataset source')
        exit(0)

    # Generate ground-truth 2D pose data
    print('\nComputing ground-truth 2D poses...')
    dataset = Human36mDataset(output_filename + '.npz')
    output_2d_poses = {}

    # Iterate through each subject and action to compute 2D projections
    for subject in dataset.subjects():
        output_2d_poses[subject] = {}
        for action in dataset[subject].keys():
            anim = dataset[subject][action]
            positions_2d = []

            # Process each camera to get 2D projections from 3D positions
            for cam in anim['cameras']:
                pos_3d = world_to_camera(anim['positions'], R=cam['orientation'], t=cam['translation'])
                pos_2d = wrap(project_to_2d, pos_3d, cam['intrinsic'], unsqueeze=True)
                pos_2d_pixel_space = image_coordinates(pos_2d, w=cam['res_w'], h=cam['res_h'])
                positions_2d.append(pos_2d_pixel_space.astype('float32'))
            output_2d_poses[subject][action] = positions_2d

    # Save the 2D positions with metadata to a compressed .npz file
    print('Saving...')
    metadata = {
        'num_joints': dataset.skeleton().num_joints(),
        'keypoints_symmetry': [dataset.skeleton().joints_left(), dataset.skeleton().joints_right()]
    }
    np.savez_compressed(output_filename_2d, positions_2d=output_2d_poses, metadata=metadata)

    print('Done.')
